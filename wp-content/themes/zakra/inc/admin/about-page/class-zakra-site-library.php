		...
		if ( isset( $demo_packages ) ) :
			...
			foreach ( $demo_packages->demos as $demo_package_demo => $demo_package_data ) {
				...
				$translation_array = array(
					'uri' => admin_url( '/themes.php?page=demo-importer' ),
				);
				wp_localize_script( 'zakra-plugin-install-helper', 'zakra_redirect_demo_page', $translation_array );

				$output .='<a class="btn-get-started button button-primary button-small preview install-demo-preview"
					   href="' . esc_url( $nonce ) .'" data-name="' . esc_attr( $slug ) . '" data-slug="'. esc_attr( $slug ) . '" aria-label="Get started with Zakra">';

				$output .= esc_html__( 'Import', 'zakra' );
				$output .= '</a>';
