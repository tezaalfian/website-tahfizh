			...
			<div class="zakra-message-wrapper">
				...
				$nonce              = add_query_arg(
					array(
						...
						'plugin_status' => 'all',
						'paged'         => '1',
						'_wpnonce'      => wp_create_nonce( 'get-started-plugin_' . $plugin_link_suffix ),
					), network_admin_url( 'plugins.php' )
				);

				$translation_array = array(
					'uri' => esc_url( admin_url( '/themes.php?page=demo-importer&zakra-hide-notice=welcome' ) ),
				);
				wp_localize_script( 'zakra-plugin-install-helper', 'zakra_redirect_demo_page', $translation_array )
				?>
				<div class="submit">
					<a class="btn-get-started button button-primary button-hero"
					   href="<?php echo esc_url( $nonce ); ?>" data-name="<?php echo esc_attr( $slug ); ?>" data-slug="<?php echo esc_attr( $slug ); ?>" aria-label="Get started with Zakra"><?php esc_html_e( 'Get started with Zakra', 'zakra' ); ?></a>
				</div>
			</div>
		</div>
		<?php
